  ...
  return (
    <div className="glass rounded-2xl p-4 shadow-glow">
      ...
      <ul className="space-y-2">
        {mine.map(p => (
          <li key={p.id} className="flex items-center justify-between gap-3">
            <div className="text-sm shrink min-w-0">
            ...
            <button
              className="rounded-lg px-3 py-1 border border-red-400/50 text-red-300 hover:bg-red-500/10 shrink-0"
      ...
      <div className="opacity-60 text-xs mt-3">
        Only pulses created in this browser appear here (token stored locally).
      </div>
    </div>
  )
}

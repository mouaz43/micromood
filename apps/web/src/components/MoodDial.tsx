...
export function MoodDial({ onSubmit, loading }:{
  onSubmit:(mood:string, energy:number, message?:string)=>void; loading?:boolean;
}) {
  ...
  return (
    <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="glass rounded-2xl p-6 shadow-glow max-w-md w-full">
      ...
      <div className="mb-2">
        ...
        <textarea value={message} onChange={e=>setMessage(e.target.value.slice(0,150))}
          placeholder="A short thought, feeling, or moment (max 150 chars)"
          className="mt-1 w-full rounded-xl bg-white/5 border border-white/10 p-3 outline-none focus:border-aurora-3" rows={3}/>
        <div className="text-xs mt-1 opacity-60 text-right">{remaining} chars left</div>
      </div>

      <div className="flex justify-end">
        <button onClick={()=> selected && onSubmit(selected, energy, message.trim()||undefined)}
